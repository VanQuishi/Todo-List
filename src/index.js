...
for (let i = 0; i < storage.projects.length; i++) {
  for (let j = 0; j < storage.projects[i].tasks.length; j++) {
    ...
    if (taskDueDate.toLocaleDateString() == todayDateObj.toLocaleDateString()) {
      todayTasks.push([
        storage.projects[i].tasks[j],
        storage.projects[i].title,
        storage.projects[i].color,
        j,
      ]);
    }
...
weekViewBtn.addEventListener("click", function () {
  ...
  for (var i = 0; i < storage.projects.length; i++) {
    for (var j = 0; j < storage.projects[i].tasks.length; j++) {
      ...
      let taskDueDate = new Date(
        `"${storage.projects[i].tasks[j].dueDateAndTime}"`,
      );
      console.log(taskDueDate.toString());
      if (
        taskDueDate <= getSundayOfCurrentWeek() &&
        taskDueDate >= getMondayOfCurrentWeek()
      ) {
  ...
  let weekView = new WeekView(
    getMondayOfCurrentWeek().toLocaleDateString(),
    getSundayOfCurrentWeek().toLocaleDateString(),
    weekTasks,
  );
...
projectViewBtn.addEventListener("click", function () {
  ...
  for (let i = 0; i < storage.projects.length; i++) {
    var projectView = new ProjectView(
      storage.projects[i].title,
      storage.projects[i].color,
      storage.projects[i].tasks,
    );
    view.appendChild(projectView.htmlDisplay);
  }

  let projectTitleButtons =
    document.getElementsByClassName("projectTitleButton");
...
function toggleList() {
  let list = document.getElementById(
    `${this.dataset.project}` + "taskListWrapper",
  );
  console.log(list);
...
newProjectButton.addEventListener("click", function () {
  if (
    ...
    projectInputForm.style.display == ""
  ) {
...
submitTaskBtn.addEventListener("click", function () {
  if (taskTitleInput.value != "" && dueDate.value != "") {
    if (submitTaskBtn.innerText == "Add Task") {
      ...
      storage.projects[projectIdx].tasks[taskIdx].description =
        taskDescription.value;
      let dueDateObj = new Date(dueDate.value.replace(/-/g, "/"));
...
function refreshEditDelTaskBtns() {
  ...
  for (var i = 0; i < editTaskButtons.length; i++) {
    editTaskButtons[i].addEventListener("click", function () {
      ...
      var todayDate = new Date(this.parentElement.dataset.duedate)
        .toISOString()
        .slice(0, 10);
...
const deleteProjectButtons =
  document.getElementsByClassName("deleteProjectBtn");
